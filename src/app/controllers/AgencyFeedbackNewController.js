...
class AgencyFeedbackNewController {
    ...
    delivery(req, res, next) {
        ...
        AgencyFeedbackNew.findById(req.params.id, function (err, docs) {
            if (err) {
                res.send(err);
            }
            else {
                // const product_lines = new Agency(docs);
                AgencyFeedbackNew.deleteOne({ _id: req.params.id })
                    .then(() => {
                        InsuranceNew .insertMany(docs);
                        res.redirect('back');
...
module.exports = new AgencyFeedbackNewController;